  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: 'var(--bg)', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ position:'absolute', top:'10%', left:'10%', width:'300px', height:'300px', borderRadius:'50%', background:'radial-gradient(circle, rgba(185,255,102,0.07) 0%, transparent 70%)', animation:'floatOrb 7s ease-in-out infinite' }} />
        <div style={{ position:'absolute', bottom:'15%', right:'5%', width:'200px', height:'200px', borderRadius:'50%', background:'radial-gradient(circle, rgba(185,255,102,0.05) 0%, transparent 70%)', animation:'floatOrb 9s ease-in-out 2s infinite' }} />
        ...
        <div style={{ position:'relative', zIndex:1, textAlign:'center', maxWidth:'380px' }}>
          {/* Mini chart preview */}
          <div style={{ background:'rgba(185,255,102,0.04)', border:'1px solid rgba(185,255,102,0.12)', borderRadius:'20px', padding:'28px', marginBottom:'36px' }}>
            <div style={{ display:'flex', alignItems:'flex-end', gap:'6px', height:'80px', marginBottom:'16px', justifyContent:'center' }}>
              {bars.map((h, i) => (
                <div key={i} style={{
                  flex:1, maxWidth:'18px',
                  height:`${h}%`,
                  background: i === bars.length - 1
                    ? 'var(--lime)'
                    : `rgba(185,255,102,${0.15 + i * 0.06})`,
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div>
                ...
                <div style={{ color:'var(--lime)', fontSize:'22px', fontWeight:700, marginTop:'4px' }}>PKR 2.4M</div>
              </div>
              <div style={{ background:'rgba(185,255,102,0.12)', border:'1px solid rgba(185,255,102,0.2)', borderRadius:'8px', padding:'6px 12px' }}>
                <div style={{ color:'var(--lime)', fontSize:'13px', fontWeight:700 }}>+18.4%</div>
          ...
          <p style={{ color:'var(--muted)', fontSize:'14px', lineHeight:1.7, marginBottom:'28px' }}>
            AI-powered insights across 68 PSX stocks.<br/>
            Track, analyze, and grow your portfolio.
          </p>

          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {['18 comprehensive modules', 'Groq AI chat assistant', 'Real-time risk analysis'].map((f, i) => (
              <div key={i} className={`stagger-${i+1}`} style={{ display:'flex', alignItems:'center', gap:'10px', background:'rgba(185,255,102,0.04)', border:'1px solid rgba(185,255,102,0.1)', borderRadius:'10px', padding:'10px 16px' }}>
                <span style={{ color:'var(--lime)', fontSize:'14px' }}>✓</span>
                <span style={{ color:'var(--muted)', fontSize:'13px' }}>{f}</span>
              </div>
            ))}
          </div>

          <div style={{ marginTop:'32px', color:'var(--muted)', fontSize:'11px', opacity:0.5 }}>
      ...
      <div style={{
        ...
        <div style={{ marginBottom:'40px' }}>
          ...
          <h1 style={{ fontSize:'28px', fontWeight:700, color:'var(--white)', margin:'0 0 6px' }}>Welcome back</h1>
        ...
        <p style={{ textAlign:'center', color:'var(--muted)', marginTop:'28px', fontSize:'14px' }}>
          Don't have an account?{' '}
          <Link to="/register" style={{ color:'var(--lime)', textDecoration:'none', fontWeight:600 }}>Create Account</Link>
        </p>

        <p style={{ textAlign:'center', color:'var(--muted)', fontSize:'12px', marginTop:'12px', opacity:0.4 }}>
      ...
      <style>{`
        ...
        @media (min-width: 860px) {
          ...
          .auth-right-panel { max-width: 480px !important; border-left: 1px solid var(--sidebar-border); }
        }
      `}</style>
    </div>
  )
